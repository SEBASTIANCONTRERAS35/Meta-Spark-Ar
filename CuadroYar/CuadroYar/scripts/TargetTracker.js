...
// @ts-nocheck
 ...
 // Encuentra el objeto en el que se mostrará el número
 S.root.findFirst('Month').then(function(textObject) {
  // Obtener la fecha actual
  const currentDate = new Date();
  
  // Definir la fecha de referencia (20 de marzo del año actual)
  const referenceDate = new Date(currentDate.getFullYear(), 2, 20); // Meses en JavaScript son 0-indexados (0 es enero, 2 es marzo)
  
  // Calcular la diferencia en meses entre la fecha actual y la fecha de referencia
  let monthsDifference = (currentDate.getFullYear() - referenceDate.getFullYear()) * 12;
  monthsDifference += (currentDate.getMonth() - referenceDate.getMonth());
  
  // Asegurarse de que el número no sea negativo
  monthsDifference = Math.max(monthsDifference, 0);
  
  // Crear el texto completo
  const displayText = `${monthsDifference} meses\ncon mi amor`;
  
  // Actualizar el texto con el nuevo número y texto adicional
  textObject.text = displayText;
  
  // Para depuración, puedes imprimir el valor en la consola
  D.log('Display Text: ' + displayText);
}).catch(function(error) {
  // Manejo de errores si no se encuentra el objeto de texto
  D.log('Error finding Month object: ' + error.message);
});
